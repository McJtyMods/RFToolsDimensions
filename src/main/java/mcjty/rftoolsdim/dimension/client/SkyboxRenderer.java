...
public class SkyboxRenderer implements ISkyRenderHandler {
    ...
    public void render(int ticks, float partialTicks, PoseStack matrixStack, ClientLevel world, Minecraft mc) {
        ...
        bufferbuilder.vertex(pose, -v, -v, -200).uv(0, 0).endVertex();
        bufferbuilder.vertex(pose, v,  -v, -200).uv(1, 0).endVertex();
        bufferbuilder.vertex(pose, v,   v, -200).uv(1, 1).endVertex();
        bufferbuilder.vertex(pose, -v,  v, -200).uv(0, 1).endVertex();
        ...
        bufferbuilder.vertex(pose, 200, v,  -v).uv(0, 0).endVertex();
        bufferbuilder.vertex(pose, 200, -v, -v).uv(1, 0).endVertex();
        bufferbuilder.vertex(pose, 200, -v,  v).uv(1, 1).endVertex();
        bufferbuilder.vertex(pose, 200, v,   v).uv(0, 1).endVertex();
