...
public abstract class AbstractRfToolsCommand implements RfToolsCommand {
    ...
    protected boolean fetchBool(ICommandSender sender, String[] args, int index, boolean defaultValue) {
        ...
        try {
            ...
            value = false;
            sender.addChatMessage(new ChatComponentText(TextFormatting.RED + "Parameter is not a valid boolean!"));
        } catch (ArrayIndexOutOfBoundsException e) {
            return defaultValue;
        }
        return value;
    ...
    protected int fetchInt(ICommandSender sender, String[] args, int index, int defaultValue) {
        ...
        try {
            ...
            value = 0;
            sender.addChatMessage(new ChatComponentText(TextFormatting.RED + "Parameter is not a valid integer!"));
        } catch (ArrayIndexOutOfBoundsException e) {
            return defaultValue;
        }
        return value;
    ...
    protected float fetchFloat(ICommandSender sender, String[] args, int index, float defaultValue) {
        ...
        try {
            ...
            value = 0.0f;
            sender.addChatMessage(new ChatComponentText(TextFormatting.RED + "Parameter is not a valid real number!"));
        } catch (ArrayIndexOutOfBoundsException e) {
            return defaultValue;
        }
        return value;
